...
<form action="<%=IMSView.POLICYIES_CTL%>" method="post">
			...
			<table class="table table-sm table-bordered">
				...
				<tbody>
					...
					<tr>
						...
						<%if(userLoggedIn){%>
						<td><a href="<%=IMSView.BUY_POLICY_CTL%>?pId=<%=bean.getId()%>">Apply</a></td>	
						<%} %>
					</tr>
					<%
					}
			...
			<nav aria-label="Page navigation example float-end">
				<ul class="pagination justify-content-end">
					<li class="page-item disabled"><input type="submit" name="operation"
								...
								<% if(i==pageNo){%>	
								<li class="page-item active"><a class="page-link activate" href="<%=IMSView.POLICYIES_CTL%>?pageNo=<%=i%>"><%=i%></a></li>
								<%}else{%>
								<li class="page-item"><a class="page-link" href="<%=IMSView.POLICYIES_CTL%>?pageNo=<%=i%>"><%=i%></a></li>
								<%} %>
								<%}%>
