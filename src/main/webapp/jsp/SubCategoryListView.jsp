...
<body>
	...
	<form action="<%=IMSView.SUB_CATEGORY_LIST_CTL%>" method="post">
	<div class="card" style="margin: 10px;">
		<h5 class="card-header"
			style="height: 50px; font-size: 25px; background-color: #18334f; color: white;">Sub Category
			List</h5>
		<div class="card-body">
			<div class="row">
				<div class="col">
				...
				<div class="col">
					<input type="submit" class="btn btn-primary btn btn-info"
						name="operation" value="<%=SubCategoryListCtl.OP_SEARCH%>">or<input
						type="submit" class="btn btn-primary btn btn-info"
						name="operation" value="<%=SubCategoryListCtl.OP_RESET%>">
				</div>
			</div>
			...
			<table class="table table-sm table-bordered">
				...
				<tbody>
					...
					<tr>
						...
						<td scope="row"><img
							src="<%=IMSView.APP_CONTEXT + IMSView.GET_IMAGE_VIEW%>?id=<%=bean.getId()%>&table=SubCategory"
							width="100px" height="100px"></td>
							<td scope="row"><%=bean.getCategoryName()%></td>
						<td scope="row"><%=bean.getName()%></td>
						<td scope="row"><%=bean.getDescription()%></td>
						<td><a href="<%=IMSView.SUB_CATEGORY_CTL%>?id=<%=bean.getId()%>"
			...
			<input type="submit" name="operation"
								...
								value="<%=SubCategoryListCtl.OP_DELETE%>"
								<%=(list.size() == 0) ? "disabled" : ""%>>
			<nav aria-label="Page navigation example float-end">
				<ul class="pagination justify-content-end">
					<li class="page-item disabled"><input type="submit" name="operation"
								class="page-link"
								value="<%=SubCategoryListCtl.OP_PREVIOUS%>"
								<%=(pageNo == 1) ? "disabled" : ""%>></li>
					<% int count=DataUtility.getInt(String.valueOf(size/10));%>
								<% for(int i=1;i<=count+1;i++){%>
								<% if(i==pageNo){%>	
								<li class="page-item active"><a class="page-link activate" href="<%=IMSView.SUB_CATEGORY_LIST_CTL%>?pageNo=<%=i%>"><%=i%></a></li>
								<%}else{%>
								<li class="page-item"><a class="page-link" href="<%=IMSView.SUB_CATEGORY_LIST_CTL%>?pageNo=<%=i%>"><%=i%></a></li>
								<%} %>
								<%}%>
					<li class="page-item"><input type="submit" name="operation"
								class="page-link"
								value="<%=SubCategoryListCtl.OP_NEXT%>"
